...
class CountryList: NSObject {
    ...
    /**
        (en): Produces the "lang={code}&" param to add to the GeoNames URL.
     
        (pt): Produces o paramêtro "lang={codigo}&" para ser adicionada a URL do GeoNames.
     
        - parameter languageCode An optional language code. (O código da língua, opcional).
     */
